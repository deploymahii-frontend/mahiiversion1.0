import {
  FiHome,
  FiRefreshCw,
  FiDownload,
  FiSearch,
  FiFilter,
  FiRepeat,
  FiUpload,
  FiEye,
} from "react-icons/fi";

export default function BankCashManagement({
  loading,
  overview = {},
  accounts = [],
  search = "",
  onSearch,
  onRefresh,
  onExport,
  onImportStatement,
  onTransfer,
  onView,
}) {
  if (loading) {
    return (
      <div className="rounded-2xl bg-white p-6 shadow-sm">
        <div className="h-[720px] animate-pulse rounded-xl bg-gray-100" />
      </div>
    );
  }

  const cards = [
    { title: "Bank Balance", value: overview.bankBalance ?? "₹0" },
    { title: "Cash Balance", value: overview.cashBalance ?? "₹0" },
    { title: "Today's Inflow", value: overview.inflow ?? "₹0" },
    { title: "Today's Outflow", value: overview.outflow ?? "₹0" },
  ];

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="rounded-2xl bg-white shadow-sm p-6 flex items-center justify-between">
        <div>
          <h2 className="flex items-center gap-2 text-2xl font-bold">
            <FiHome />
            Bank & Cash Management
          </h2>

          <p className="text-gray-500">
            Manage bank accounts, cash balances, transfers, and reconciliations.
          </p>
        </div>

        <div className="flex gap-3">
          <button onClick={onRefresh} className="rounded-lg border p-3">
            <FiRefreshCw />
          </button>

          <button
            onClick={onImportStatement}
            className="rounded-lg border px-5"
          >
            <FiUpload className="mr-2 inline" />
            Import Statement
          </button>

          <button
            onClick={onExport}
            className="rounded-lg bg-green-600 px-5 text-white"
          >
            <FiDownload className="mr-2 inline" />
            Export
          </button>
        </div>
      </div>

      {/* KPI */}
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          <div key={card.title} className="rounded-2xl bg-white shadow-sm p-5">
            <p className="text-sm text-gray-500">{card.title}</p>
            <h3 className="mt-3 text-3xl font-bold">{card.value}</h3>
          </div>
        ))}
      </div>

      {/* Search */}
      <div className="rounded-2xl bg-white shadow-sm p-4">
        <div className="flex gap-4">
          <div className="relative flex-1">
            <FiSearch className="absolute left-3 top-3 text-gray-400" />

            <input
              value={search}
              onChange={(e) => onSearch?.(e.target.value)}
              placeholder="Search bank account..."
              className="w-full rounded-lg border py-2 pl-10 pr-4"
            />
          </div>

          <button className="rounded-lg border px-5">
            <FiFilter className="mr-2 inline" />
            Filter
          </button>
        </div>
      </div>

      {/* Accounts */}
      <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
        <table className="w-full">
          <thead className="bg-gray-100">
            <tr>
              <th className="p-4 text-left">Account</th>
              <th className="p-4 text-left">Bank</th>
              <th className="p-4 text-right">Balance</th>
              <th className="p-4 text-left">Currency</th>
              <th className="p-4 text-left">Actions</th>
            </tr>
          </thead>

          <tbody>
            {accounts.map((account) => (
              <tr key={account.id} className="border-t">
                <td className="p-4">{account.accountName}</td>
                <td className="p-4">{account.bankName}</td>
                <td className="p-4 text-right">{account.balance}</td>
                <td className="p-4">{account.currency}</td>

                <td className="p-4">
                  <div className="flex gap-2">
                    <button
                      onClick={() => onView?.(account)}
                      className="rounded border p-2"
                    >
                      <FiEye />
                    </button>

                    <button
                      onClick={() => onTransfer?.(account)}
                      className="rounded border p-2"
                    >
                      <FiRepeat />
                    </button>
                  </div>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>

      {/* Treasury Workflow */}
      <div className="rounded-2xl border border-dashed p-8 text-center text-gray-500">
        Receipt → Bank/Cash → Reconciliation → Transfer → General Ledger
      </div>
    </div>
  );
}
