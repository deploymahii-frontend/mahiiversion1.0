...
export default function ExpenseManagement({
  ...
  const cards = [
    { title: "Total Expenses", value: overview.total ?? "₹0" },
    { title: "Pending Approval", value: overview.pending ?? 0 },
    { title: "Approved", value: overview.approved ?? 0 },
    { title: "Reimbursed", value: overview.reimbursed ?? "₹0" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-white shadow-sm p-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">
            Manage business expenses, approvals, reimbursements, and receipts.
          </p>
        </div>
          ...
          <tbody>
            {expenses.map((expense) => (
              <tr key={expense.id} className="border-t">
                <td className="p-4">{expense.employee}</td>
                <td className="p-4">{expense.category}</td>
                <td className="p-4 text-right">{expense.amount}</td>
      ...
      <div className="rounded-2xl border border-dashed p-8 text-center text-gray-500">
        Expense Claim → Receipt → Approval → Reimbursement → General Ledger
      </div>
    </div>
  );
}
